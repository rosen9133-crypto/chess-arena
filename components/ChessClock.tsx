...
function ClockRow({
  label, icon, time, active, isUntimed,
}: {
  label: string;
  icon: string;
  time: number;
...
export default function ChessClock({
  ...
  const whiteClock = (
    <ClockRow
      ...
      icon="⚪"
      time={whiteTime}
      active={!isUntimed && isClockRunning && activeClock === "w"}
      isUntimed={isUntimed}
    />
  );
  ...
  const blackClock = (
    <ClockRow
      ...
      icon="⚫"
      time={blackTime}
      active={!isUntimed && isClockRunning && activeClock === "b"}
      isUntimed={isUntimed}
    />
  );
  ...
  return (
    <div className="rounded-xl border border-slate-700 bg-slate-800 p-4">
      <h2 className="mb-4 text-center text-lg font-bold">⏱ Chess Clock</h2>
